...
const loadKeys = async () => {
  ...
  try {
    const { data } = await api.get(`/v1/pgp/users/${user.value.id}/keys`)
    keys.value = data
  } catch (err) {
    error.value = err.response?.data?.error || 'Failed to load PGP keys'
  } finally {
    loading.value = false
  }
}
...
const importKey = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.error || 'Failed to import key'
  } finally {
    loading.value = false
  }
}
...
const setPrimary = async (keyId) => {
  ...
  try {
    await api.post(`/v1/pgp/keys/${keyId}/primary`)
    success.value = 'Primary key updated'
    await loadKeys()
  } catch (err) {
    error.value = err.response?.data?.error || 'Failed to set primary key'
  } finally {
    loading.value = false
  }
}
...
const deleteKey = async (keyId) => {
  ...
  try {
    await api.delete(`/v1/pgp/keys/${keyId}`)
    success.value = 'PGP key deleted'
    await loadKeys()
  } catch (err) {
    error.value = err.response?.data?.error || 'Failed to delete key'
  } finally {
    loading.value = false
  }
}
