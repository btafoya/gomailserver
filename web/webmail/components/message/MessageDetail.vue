...
onMounted(() => {
  ...
  // Register message action shortcuts
  registerShortcut({
    key: 'r',
    handler: () => handleReply(),
    description: 'Reply'
...
const handleDelete = async () => {
  if (confirm('Delete this message?')) {
    ...
    router.push(`/mail/${message.value.mailbox_id}`)
  }
}

const handleReply = () => {
  router.push(`/mail/compose?reply=${props.messageId}`)
}

const handleReplyAll = () => {
  router.push(`/mail/compose?replyAll=${props.messageId}`)
}

const handleForward = () => {
  router.push(`/mail/compose?forward=${props.messageId}`)
}
