...
func (r *webhookRepository) Update(ctx context.Context, webhook *domain.Webhook) error {
	query := `
		UPDATE webhooks
		SET name = $1, url = $2, secret = $3, event_types = $4, active = $5, description = $6, updated_at = $7
		WHERE id = $8
	`
...
func (r *webhookRepository) UpdateDelivery(ctx context.Context, delivery *domain.WebhookDelivery) error {
	query := `
		...
		SET attempt_count = $1, status = $2, status_code = $3, response_body = $4,
			error_message = $5, next_retry_at = $6, first_attempted_at = $7,
			last_attempted_at = $8, completed_at = $9
		WHERE id = $10
	`
